# Parameters
output_csv = "output_expr_configurable.csv"
...
with open(output_csv, "w") as f:
    ...
    for bit in range(num_bits):
        for run in range(iterations):
            name = f"E{bit}_iter{run}"
            expr = f'value(value(getData("/out0<{bit}>" ?result "tran") "Iteration" {run+1}) {sample_time})'
            line = f"{name},expr,{expr},point,t,,\n"
            f.write(line)

print(f"✅ Wrote CSV with {num_bits} bits × {iterations} iterations at {sample_time}s.")
